...
function playChime() {
  const chime = new Audio(browser.runtime.getURL('audio/chime.mp3'));
...
function startTimer(sessionDurationMinutes, chimeIntervalSeconds) {
  ...
  timerId = setInterval(() => {
    ...
    if (timerState.chimeInterval > 0) {
      ...
      const timeInCurrentChimeCycle = timeSinceChimeLoopStart % (timerState.chimeInterval * 1000);
